...
public class SuccessesManager implements ISuccessesManagerLocal, ISuccessesManagerRemote {
  ...
  /**
   * Retrieve all Success acquired by an User. This method will NOT use the link
   * between User and Success, it will search for Events belonging to the User
   * and compute the amount of points acquired by ActionType to identify all the
   * passed Success.
   *
   *
   * @param id
  ...
  public List<Success> findAllAcquiredByUser(long id) {
    /**
     * TODO rewrite with NamedQuery ...
     * Meaning of this query.
     *
     * Part 1: The query will take each ActionType and calculate the amount of
     * points needed (based on the Rules attached to this ActionType) to achieve
     * a Success. The result will be a list containing each Success id and for
     * each of them, the amount of points needed in a specific ActionType to
     * pass the Success.
     *
     * Part 2: The query will search for a given user, all the events and will
     * compute (based on the ActionType of each Event) the amount of points the
     * user has accumulated for each ActionType.
     *
     * Part 3: With a left join, the query will retrieve a list of Success id
     * and for each of them an extra information (1 or 0). To sum up, this extra
     * information represent, for a Success, if a set of rules has been passed
     * or not. A set of rules represent the rules that are related to the same
     * ActionType for a same Success.
     *
     * Part 4: The query will retrieve the passed Successes for a given User.
     * Based on the previous query, it will verify if, for a Success, each set
     * of rules have been passed or not. For this, it will compare the number of
     * different ActionType for a Success with the number of passed
     * group-of-rules.
     *
     */
    String query = // Part4
